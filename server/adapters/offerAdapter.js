...
const prepareUrl = (url) =>
  url && !url.startsWith('http') ? `${getBaseUrl()}${url.startsWith('/') ? '' : '/'}${url}` : url;
...
const adaptFullOfferToClient = (offer, author) => {
  ...
  return {
    ...
    host: {
      ...
      avatarUrl: prepareUrl(author?.avatar || ''),
    },
    images: Array.isArray(offer.photos) ? offer.photos.map((photo) => prepareUrl(photo)) : [],
